...
LOG_DIR=${LOG_DIR:-/var/log/elastisys/autoscaler}
mkdir -p ${LOG_DIR}
...
# where autoscaler instance runtime state is kept
STORAGE_DIR=${STORAGE_DIR:-/var/lib/elastisys/autoscaler/instances}
# add-on subsystems that will be added to each created autoscaler instance
ADDONS_CONFIG=${ADDONS_CONFIG:-/etc/elastisys/autoscaler/addons/addons.json}

RUNTIME_OPTS="--storage-dir ${STORAGE_DIR} --addons-config ${ADDONS_CONFIG}"
