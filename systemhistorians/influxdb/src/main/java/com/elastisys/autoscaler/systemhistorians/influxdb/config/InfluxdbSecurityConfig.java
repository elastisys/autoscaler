 ...
 * server.
 *
 * @see InfluxdbSystemHistorianConfig
 */
public class InfluxdbSecurityConfig {
    ...
    /**
     * Creates a {@link InfluxdbSecurityConfig}.
     *
     * @param https
     *            If the InfluxDB server runs with HTTPS enabled, this option is
     ...
     *            This option is only relevant when the server runs HTTPS.
     */
    public InfluxdbSecurityConfig(Boolean https, BasicCredentials auth, Boolean verifyCert, Boolean verifyHost) {
        this.https = https;
        this.auth = auth;
    ...
    public boolean equals(Object obj) {
        if (obj instanceof InfluxdbSecurityConfig) {
            InfluxdbSecurityConfig that = (InfluxdbSecurityConfig) obj;
            return Objects.equals(useHttps(), that.useHttps()) //
